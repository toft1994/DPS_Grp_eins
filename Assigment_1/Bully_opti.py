...
def run_node(node_id, starter, max_node_id, SHARED_QUEUE_INFO):
    ...
    SHARED_THREAD_MSGS['OK_RECECIVED'] = False
...
def election(node_id, max_node_id, send_queue, SHARED_THREAD_MSGS):
    if node_id == max_node_id-1:
        ...
        send_message("TERMINATE", str(max_node_id-1), node_id, send_queue)
    else:
        print("Node id: " + str(node_id) + " sent election msg to node id " +
              str(max_node_id-1))
       
        send_message("ELECTION", max_node_id-1, node_id, send_queue)
        time.sleep(0.1)
        if SHARED_THREAD_MSGS['OK_RECECIVED'] != True:
            election(node_id, max_node_id-1, send_queue, SHARED_THREAD_MSGS)

def receive_message(node_id, receiver_queue, send_queue, SHARED_THREAD_MSGS):
    while SHARED_THREAD_MSGS['TERMINATE_MESSAGE'] == False:
        try:
            ...
            if(msg['type'] == "ELECTION"):
                ...
                SHARED_THREAD_MSGS['OK_RECECIVED'] = True
                SHARED_THREAD_MSGS['ELECTION_ACTIVE'] = False
